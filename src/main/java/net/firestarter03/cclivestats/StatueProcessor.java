...
public class StatueProcessor {
    private static final Path STATUE_LOG_FILE = Path.of("cc_live_statues.txt");

    //Regex Patterns
    private static final Pattern NAME_PATTERN = Pattern.compile("^\\(.*?\\) \\[Statue\\]$");
    ...
    public void process(String hoverText) {
        ...
        if (lines.length >= 2) {
            ...
            if (nameMatcher.find() && levelMatcher.find()) {
                ...
                writeToFile("Statue: " + name + ", Level: " + level);
                System.out.println("Statue hinzugefügt: " + name + ", Level: " + level);
            }
        }
    }

    private void writeToFile(String message) {
        try (FileWriter writer = new FileWriter(STATUE_LOG_FILE.toFile(), true)) {
